...
if __name__ == "__main__":
    ...
    NODE1_IPV4 = os.getenv("NODE1_IPV4") # próximo nó no anel
    CONNECTION_PORT = int(os.getenv("CONNECTION_PORT"))

    process = Process(3, False, False) # inicializa processo com token (nó inicial)
    
    print(f"Starting mutex server on {HOST}:{CONNECTION_PORT}...")
    serverThread = threading.Thread(target=process.serverHandler, args=(HOST,CONNECTION_PORT,8001,1024,NODE1_IPV4))
    serverThread.start()
    
    if(process.getStart() == True):
        print("Initiating token circulation...")
        clientThread = threading.Thread(target=process.sendMessage, args=(NODE1_IPV4,8001))
        clientThread.start()
