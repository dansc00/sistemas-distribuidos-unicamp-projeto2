...
if __name__ == "__main__":
    ...
    process = Process(3, False, False) # inicializa processo
    
    serverThread = threading.Thread(target=process.serverHandler, args=(HOST,8000,1024,'172.18.0.2'))
    serverThread.start()

    if(process.getStart() == True):
        start = input("iniciar compartilhamento de recurso: ")
        if(start == "ok"):
            clientThread = threading.Thread(target=process.sendMessage, args=('172.18.0.2',8000))
            clientThread.start()
            clientThread.join()
