...
class ServerConnectionHandler:
	...
	def lamportClockAlgorithm(message):
		...
		currentLamportClock = process.process2.getLamportClock()

		if(message['lamportClock'] >= currentLamportClock):

			process.process2.setLamportClock(message['lamportClock'] + 1)
		else:

			process.process2.setLamportClock(currentLamportClock + 1)
		
		response = {'lamportClock': process.process2.getLamportClock()}
