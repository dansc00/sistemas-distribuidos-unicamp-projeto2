...
class Process:
    ...
    def __init__(self, lamportClock, timeRate, numEvents, serverFlag, mutexFlag, sharedFile, ID, HOST, PORT, DATA_PAYLOAD):
        ...
        self.mutexFlag = mutexFlag # flag marca o estado atual do mutex do processo
    ...
    def getMutexFlag(self):
        
        return self.mutexFlag
    
    def setMutexFlag(self, newMutexFlag):

        self.mutexFlag = newMutexFlag
    ...
    def handleReceive(self, conn):
        ...
        while(True):
            ...
            try:
                ...
                print("")
                response = self.mutexVerify(dataJson)
    ...
    def mutexVerify(self, message):
        
        i = 0
        while(i <= 5):

            if(self.getMutexFlag() == 1):
                
                try:
                    
                    with open("/recursos/shared_file.txt", "w") as file:
                        file.write("Processo{} está com acesso ao recurso\n".format(self.ID))
                except Exception as e:
                ...
                continue
            elif(self.getMutexFlag() == -1):

                break
            elif(self.getMutexFlag() == 0):

                if(self.getLamportClock() < message['lamportClock']):

                    return "OK"
                elif(self.getLamportClock() > message['lamportClock']):

                    self.setMutexFlag(1)
                    break
                elif(self.getLamportClock() == message['lamportClock']):

                    if(self.ID < message['senderID']):
...
if __name__ == "__main__":
    ...
    try:
        ...
        with open("/recursos/shared_file.txt", "r") as file:
            sharedFile = file.read()
    except Exception as e:

        print("Falha ao ler o arquivo: {}".format(e))

    process1 = Process(0, 0, 0, False, 0, sharedFile, 1, HOST, 8000, 1024) # inicializa processo
